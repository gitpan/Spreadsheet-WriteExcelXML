package Spreadsheet::WriteExcelXML::Utility;
...
This document refers to version 0.03 of Spreadsheet::WriteExcelXML::Utility, released March, 2002.



=head1 SYNOPSIS

Functions to help with some common tasks when using Spreadsheet::WriteExcelXML.
...
This module provides a set of functions to help with some common tasks encountered when using the Spreadsheet::WriteExcelXML module. The two main categories of function are:
...
Excel on Windows and the Macintosh will convert automatically between one system and the other. By default Spreadsheet::WriteExcelXML uses the 1900 format. To use the 1904 epoch you must use the C<set_1904()> workbook method, see the Spreadsheet::WriteExcelXML documentation.
...
Note, a date or time in Excel is like any other number. To display the number as a date you must apply a number format to it: Refer to the C<set_num_format()> method in the Spreadsheet::WriteExcelXML documentation:
...
See also the C<set_1904()> workbook method in the Spreadsheet::WriteExcelXML documentation.
