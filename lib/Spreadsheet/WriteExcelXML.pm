...
use strict;
use Spreadsheet::WriteExcelXML::Workbook;
...
sub new {
    ...
    my $class = shift;
    my $self  = Spreadsheet::WriteExcelXML::Workbook->new(@_);
...
The C<Spreadsheet::WriteExcelXML> module can be used to create an Excel file in XML format. The Excel XML format is supported in Excel 2002 and 2003.
...
C<Spreadsheet::WriteExcelXML> uses the same interface as C<Spreadsheet::WriteExcel>.
...
=head1 Spreadsheet::WriteExcelXML and Spreadsheet::WriteExcel

C<Spreadsheet::WriteExcelXML> uses the same interface as the C<Spreadsheet::WriteExcel> module which produces an Excel file in binary format.
...
Password protection. (supported by Spreadsheet::WriteExcel)

=item *

Embedded images. (supported by Spreadsheet::WriteExcel)
...
The reason for this is that Spreadsheet::WriteExcelXML relies on Perl's C<DESTROY> mechanism to trigger destructor methods in a specific sequence. This may not happen in cases where the Workbook, Worksheet and Format variables are not lexically scoped or where they have different lexical scopes.
...
Spreadsheet::WriteExcelXML stores dates in the 1900 format by default. If you wish to change this you can call the C<set_1904()> workbook method. You can query the current value by calling the C<get_1904()> workbook method. This returns 0 for 1900 and 1 for 1904.
...
The default code page or character set used by Spreadsheet::WriteExcelXML is ANSI. This is also the default used by Excel for Windows. Occasionally however it may be necessary to change the code page via the C<set_codepage()> method.
...
Note, you cannot currently create named ranges with C<Spreadsheet::WriteExcelXML>.
...
Note, the worksheet level password in Excel provides very weak protection. It does not encrypt your data in any way and it is very easy to deactivate. Therefore, do not use the above method if you wish to protect sensitive data or calculations. However, before you get worried, Excel's own workbook level password protection does provide strong encryption in Excel 97+. For technical reasons this will never be supported by C<Spreadsheet::WriteExcelXML>.
...
For more information refer to the documentation on formatting in the C<doc> directory of the Spreadsheet::WriteExcelXML distro, the Excel on-line help or to the tutorial at: http://support.microsoft.com/support/Excel/Content/Formats/default.asp and http://support.microsoft.com/support/Excel/Content/Formats/codes.asp
...
By default Spreadsheet::WriteExcelXML uses the Windows/1900 format although it generally isn't an issue since Excel on Windows and the Macintosh will convert automatically between one system and the other. To use the 1904 epoch you must use the C<set_1904()> workbook method.
...
The C<Spreadsheet::WriteExcelXML::Utility> module that is included in the distro contains helper functions for dealing with dates and times in Excel, for example:
...
However, this carries a performance overhead in Spreadsheet::WriteExcelXML due to the parsing of the formula and it shouldn't be used for programs that deal with a large number of dates, unless you use it in conjunction with C<store_formula()> and C<repeat_formula()> .
...
Grouping in C<Spreadsheet::WriteExcelXML> is achieved by setting the outline level via the C<set_row()> and C<set_column()> worksheet methods:
...
=item Filename required by Spreadsheet::WriteExcelXML->new()
...
Please do not send any patches without contacting the author first.
